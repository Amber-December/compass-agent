#!/usr/bin/env python3
"""
Compass Knowledge Graph Builder — 构建知识图谱可视化

基于 llm-wiki-agent 的 build_graph.py 直接复用：
- 两-pass 建图：显式 wikilink + LLM 语义推断
- 输出 workspace/knowledge/graph/graph.html 可交互可视化
- Louvain 社区检测
- SHA256 缓存增量更新
...
Usage:
    ...
    python tools/build_graph.py --open        # 构建后在浏览器打开
...
def _find_repo_root() -> Path:
    ...
    for parent in [p.parent, *p.parents]:
        if (parent / "openclaw.json").exists():
            return parent
    # fallback: 使用当前文件的上两级（兼容旧路径）
    return p.parent.parent

REPO_ROOT = _find_repo_root()
WIKI_DIR = REPO_ROOT / "workspace" / "knowledge" / "wiki"
GRAPH_DIR = REPO_ROOT / "workspace" / "knowledge" / "graph"
...
"""
        ...
        valid_rels = []
        try:
            raw = call_llm(prompt, "LLM_MODEL", "kimi-k2-6", max_tokens=1024)
            raw = raw.strip()
...
<head>
<meta charset="UTF-8">
<title>Compass 知识图谱 — Knowledge Graph</title>
<script src="https://unpkg.com/vis-network/standalone/umd/vis-network.min.js"></script>
<style>
  body {{ margin: 0; background: #1a1a2e; font-family: 'Inter', 'PingFang SC', 'Microsoft YaHei', sans-serif; color: #eee; }}
  #graph {{ width: 100vw; height: 100vh; }}
  #controls {{
    position: fixed; top: 10px; left: 10px; background: rgba(10,10,30,0.88);
    padding: 14px; border-radius: 10px; z-index: 10; max-width: 300px;
...
<div id="controls">
  ...
  <p>点击节点高亮其邻居并在右侧展示内容。点击背景恢复全图。</p>
</div>
<div id="graph"></div>
<aside id="drawer">
...
// ── 孤儿节点（无连接）固定在外围，避免在中央乱飘 ──
const orphanRadius = 1200;
const orphanNodes = originalNodes.filter(n => n.value <= 1);
orphanNodes.forEach((node, i) => {{
  const angle = (i / Math.max(orphanNodes.length, 1)) * 2 * Math.PI - Math.PI / 2;
  node.x = orphanRadius * Math.cos(angle);
  node.y = orphanRadius * Math.sin(angle);
  node.fixed = {{ x: true, y: true }};
}});

const nodeCount = originalNodes.length;
const gravConst = nodeCount > 80 ? -20000 : nodeCount > 30 ? -12000 : -6000;
const springLen = nodeCount > 80 ? 400 : nodeCount > 30 ? 300 : 220;
...
def build_graph(infer: bool = True, open_browser: bool = False, clean: bool = False,
    ...
    print(f"  saved: workspace/knowledge/graph/graph.json  ({len(nodes)} nodes, {len(edges)} edges)")

    html = render_html(nodes, edges)
    GRAPH_HTML.write_text(html, encoding="utf-8")
    print(f"  saved: workspace/knowledge/graph/graph.html")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build Compass knowledge graph")
    ...
    parser.add_argument("--save", action="store_true", help="Save report to workspace/knowledge/graph/graph-report.md")
