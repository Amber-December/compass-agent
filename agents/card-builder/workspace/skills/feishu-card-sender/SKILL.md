...
## Markdown template → chart card sending convention

When the user provides a template-like `.md` report and expects a card back, this skill should be the default sending path after the card is authored.

### Recommended send shape for report cards
- title + period
- 3-4 KPI metric blocks
- one `chart` bar block for comparison
- one `chart` line block for trends
- short risk summary
- optional CTA button

### Chart sending rule
- Prefer real `tag: "chart"` components when the user explicitly requests interactive charts.
- For comparison views, prefer `chart_spec.type: "bar"`.
- For daily trend views, prefer `chart_spec.type: "line"`.
- Keep `legends.visible: true` when legend switching is desired.
- If needed, send minimal experiment cards first to verify recipient-client rendering.

### Compatibility rule
- Treat API success and client render success as different things.
- If the card sends successfully but the client degrades to an upgrade placeholder or fallback view, report that honestly.
- Once a specific recipient/client combination is known to render `chart` correctly, reuse the chart-card path for future markdown-driven report sends.
