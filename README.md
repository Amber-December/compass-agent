...
weekly-reporter 调用 card-builder 将 Markdown 渲染为飞书消息卡片，直接推送到群聊。卡片中点击"查看详情"，即可打开完整的可交互前端页面：
...
详情页包含数据看板（Chart.js 可视化）、风险分析、任务追踪、知识推荐等模块，支持周环比切换、数据下钻、图谱联动。
...
Compass 的架构与数据底座设计并不局限于 MCN 领域，其核心能力可迁移至任何**项目制协作场景**：

| 场景 | 适配方式 |
|------|----------|
| **软件开发团队** | 接入 GitHub/Jira/Confluence，自动同步需求文档、代码规范、迭代周报 |
| **咨询/律所** | 接入内部知识库，case 检索、客户数据查询、项目进度看板 |
| **消费品品牌** | 接入飞书/钉钉，整合产品资料、渠道数据、营销活动复盘 |
| **教育机构** | 接入课程 Wiki、学生档案、教学周报，实现教务知识问答 |
| **研发实验室** | 接入实验记录系统，文献检索、实验数据查询、项目周报推送 |

只要存在"分散在多个系统中的文档 + 结构化数据 + 群聊协作"的场景，Compass 的多 Agent 架构和符号化知识编译均可直接复用。
